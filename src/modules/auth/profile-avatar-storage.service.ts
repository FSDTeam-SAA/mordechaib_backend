...
export class ProfileAvatarStorageService {
  ...
  async upload(userId: string, file: Express.Multer.File) {
    ...
    if (!file.buffer?.length) {
      throw new BadGatewayException('Avatar upload did not include image data');
    }
    const folder = this.config
      .get<string>('cloudinary.profileAvatarFolder', 'noltra/avatars')
      .replace(/^\/+|\/+$/g, '');
    try {
      const response = await this.uploadBuffer(file.buffer, {
        ...
        tags: ['noltra-profile-avatar'],
      });
      if (!response.secure_url) {
        throw new Error('Cloudinary returned no secure avatar URL');
      }
      return response.secure_url;
    } catch (error) {
      if (error instanceof BadGatewayException) throw error;
      this.logger.error(
        `Cloudinary avatar upload failed: ${this.safeErrorMessage(error)}`,
      );
      throw new BadGatewayException('Profile avatar upload failed');
    }
  }
  ...
  private assertConfigured() {
    if (
      ...
      throw new ServiceUnavailableException(
        'Cloudinary profile avatar storage is not configured',
      );
    }
  }
